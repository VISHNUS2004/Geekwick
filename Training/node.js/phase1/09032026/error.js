...
function sendStudentNotFound(res) {
...
const server = http.createServer(async (req, res) => {
	...
	if (method === "GET" && url === "/") {
		return sendJson(res, 200, {
			message: "Task 8 Error Handling API is running",
			routes: [
				"GET /students",
				"GET /students/:id",
				"POST /students",
	...
	if (method === "GET" && studentIdMatch) {
		...
		if (!student) {
			return sendStudentNotFound(res);
		}
	...
	if (method === "POST" && url === "/students") {
		try {
			...
			if (!name || !branch) {
				return sendJson(res, 400, { message: "name and branch are required" });
			}
